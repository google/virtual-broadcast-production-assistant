...
def test_websocket_valid_token(mock_verify_id_token, mock_start_agent_session, client):
    ...
    mock_start_agent_session.return_value = (AsyncMock(), mock_live_request_queue, "test_session")
    with client.websocket_connect(
        "/ws/test_user?is_audio=false&rundown_agent=cuez", subprotocols=["valid_token"]
    ) as websocket:
        websocket.close()
