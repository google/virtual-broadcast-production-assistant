...
| `som-v0.3.1-delivery-media-available.schema.json` | §3.3 | The TAMS junction event: `flow_id` (pattern `tams:flow/…`) + arrival metadata. |
| `som-v0.3.1-story-context.schema.json` | §3.3, §4, §5 | Point update of the released story-context schema, generated from it programmatically: adds `asset.media_refs[]`, `asset.voice_count`, `compliance[].media_range`. Everything else byte-identical in intent to the release. |
