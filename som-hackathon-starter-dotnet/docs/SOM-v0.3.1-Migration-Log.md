...
## 2026-08-18 (2) — Janet's beat6 finding fixed at source, and the rules that catch it

- 📌 **What she found** (17 Aug, reviewing the v0.3.2 full reference): the normative `story-context-v0.3.2-hurricane-beat6.json` carried a single FEMA `editorial_source` with `received_at` 16:20 while beat 01 published it at 13:10 — three hours before it arrived — and a `content_refs` entry with an `wire.ap.org` uri keyed to `src-fema-presser`, so pointer and id disagreed. **Both had already shipped** in the 13/14/15 Aug partner packs, and `validate.py` passed them every time: neither is expressible in JSON Schema.
- ✅ **Fixed at source, not in the output.** The run is generated by `build_hurricane_run.py`, and beat 07 *is* the shipped beat6 example — so the generator was the fix point. `editorial_source` now **accumulates** the way the reference implementation already did it (`seed-stories/06` carries two entries): AP wire at beats 01–03, FEMA joins at 04 when the presser lands (16:20, before the 16:26 snapshot), both carry to 07. `content_refs` re-keyed to `src-ap-wire` so the pointer matches its own uri. Beat 01 re-worded per Janet — the story management system mints the story, not the wire gateway. Run regenerated; beat 07 verified byte-identical to the normative example.
- ✅ **`build_hurricane_run.py` made runnable in place.** It resolved its pack path as `HERE/schema/v0.3.2-proposed` while living *inside* that directory, so it only ran from a copy at the tree root. Now falls back to `HERE`.
- ✅ **Two semantic rules added to `validate_sequence.py`**, the ones promised in the reply: `source_id` must resolve to a declared `editorial_source`, and no source may be published before its own `received_at`. Plus a heuristic tier for the case that actually shipped — a reference that resolves to the *wrong* source (intact pointer, wrong meaning), compared on provider names against the uri and content_format tokens; warns rather than fails, because it is a heuristic.
- ✅ **Mutation-tested against the original data.** Regenerated the pre-fix run from the pre-fix generator: the new rules report **2 errors + 6 warnings** on it while all 7 snapshots still pass schema validation — which is Janet's point made mechanical. The fixed run is **0 errors, 0 warnings**, so no false positives.
- ⏳ **Still owed to Janet, not in this change:** beat 08. The run ends with `usage[]` committed but never publishes the `link.committed` that is supposed to write it, so it demonstrates Story and Asset and never exercises Telling. That is a new beat, not a correction, and it is hers to shape.
