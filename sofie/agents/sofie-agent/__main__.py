...
def main(host: str, port: int):
    ...
    if os.getenv("GOOGLE_GENAI_USE_VERTEXAI"
                 ) != "TRUE" and not os.getenv("GOOGLE_API_KEY"):
        raise ValueError("GOOGLE_API_KEY environment variable not set and "
    ...
    if not skills:
        logging.warning(
            "No skills retrieved from MCP server, using default skills")
        skills = get_default_skills()
    else:
        logging.info(
            f"Successfully retrieved {len(skills)} skills from MCP server")
        for skill in skills:
