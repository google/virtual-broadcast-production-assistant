...
class TestExecuteTool:
  ...
  async def test_execute_tool_success(self, mock_get):
    ...
    mock_resp = await self.mock_response(200, '{"success": true}',
                                         {"success": True})
  ...
  async def test_execute_tool_success_with_path_slash_and_params(
    ...
    result = await execute_tool("test_tool2", {
        "path": "/resource",
        "param1": "value1"
    })
    assert result == {"success": True}
  ...
  async def test_get_secret_missing_project_id(self):
    with pytest.raises(ConfigurationError,
                       match="PROJECT_ID environment variable is not set"):
      get_secret("test_secret")
  ...
  async def test_api_config_initialise_development_no_key(self):
    ...
    with pytest.raises(
        ConfigurationError,
        match="No API key available from Secret Manager or environment."):
      await api_config.initialize()
