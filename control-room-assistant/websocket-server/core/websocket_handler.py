...
async def handle_client_messages(websocket: Any,
  ...
  try:
      ...
      try:
        ...
        if "type" in data:
          if data["type"] == "audio":
            ...
            await session.genai_session.send(input=data.get("data"),
                                             end_of_turn=True)
    ...
    if "connection closed" not in str(
        e).lower():  # Don't log normal connection closes
      logger.error(f"WebSocket connection error: {e}")
      logger.error(f"Full traceback:\n{traceback.format_exc()}")
    raise  # Re-raise to let the parent handle cleanup


async def handle_gemini_responses(websocket: Any,
                                  session: SessionState) -> None:
      ...
      async for response in session.genai_session.receive():
        try:
          ...
          if 'data=' in debug_response and 'mime_type=\'audio/pcm' in debug_response:
            debug_response = debug_response.split(
                'data=')[0] + 'data=<audio data>' + debug_response.split(
                    'mime_type=')[1]
          ...
          await process_server_content(websocket, session,
...
async def process_tool_queue(queue: asyncio.Queue, websocket: Any,
...
async def process_server_content(websocket: Any, session: SessionState,
                                 server_content: Any):
  ...
  try:
    ...
    async with await create_gemini_session() as gemini_session:
      ...
      try:
        ...
        if "code = 1006" in str(e) or "connection closed abnormally" in str(
            e).lower():
          logger.info(
